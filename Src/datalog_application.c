...
uint32_t tau = 2000, tauMAX = 10000;
int x_threshold = 100;
int y_threshold = -100;
int static_threshold = 80;
...
float velocity_y = 0, prev_velocity_y = 0;
float displacement_y = 0, displacement_prev_y = 0;
...
float velocity_filter_y, velocity_filter_prev_y, velocity_prev_y;

float accel_y_filter, accel_y_prev = 0, accel_y_filter_prev = 0;
float displacement_filter_y = 0, displacement_filter_prev_y = 0;
    ...
    if(SendOverUSB) /* Write data on the USB */
    {
    	 ...
    	 */
    		...
    		velocity_filter = velocity*iirh_0 + velocity_prev*iirh_1 - velocity_filter_prev*iirh_2;
      		...
      		velocity_filter_y = velocity_y*iirh_0 + velocity_prev_y*iirh_1 - velocity_filter_prev_y*iirh_2;
      		velocity_filter_prev_y = velocity_filter_y;
    		...
    		displacement_y = displacement_y + (velocity_filter_y + velocity_filter_prev_y)*Tsample/2;
    		velocity_prev_y = velocity_y;
			...
			displacement_filter_y = displacement_y*iirh_0 + displacement_prev_y*iirh_1 - displacement_filter_prev_y*iirh_2;
			displacement_prev_y = displacement_y;
			displacement_filter_prev_y = displacement_filter_y;
			...
			floatToInt(velocity_filter_y, &d1_vy, &d2_vy, 4);
			floatToInt(displacement_filter_y, &d1_dfy, &d2_dfy, 4);
			...
			sprintf( dataOut, "%i\t%i\r\n",
					(int)d1_df, (int)d1_dfy);
    		CDC_Fill_Buffer(( uint8_t * )dataOut, strlen( dataOut ));

    		if (d1_dfy < y_threshold && abs(d1_df) < static_threshold && *state == 0 && (msTick - *msTickStateChange) > tau)
    		{
    			*msTickStateChange = msTick;
    			*state = 1;
    			//sprintf(dataOut, "\t\t HALF WAY THERE!!! \n" );
    			//CDC_Fill_Buffer((uint8_t*)dataOut, strlen(dataOut));
    		}
    		else if (*state == 1 && (d1_df < (-static_threshold) /*|| abs(d1_dfy) > static_threshold*/ || (msTick - *msTickStateChange) > tauMAX) )
    		{
    			*msTickStateChange = msTick;
    			*state = 0;
    			//sprintf(dataOut, "\t\t GO BACK!!! \n" );
    			//CDC_Fill_Buffer((uint8_t*)dataOut, strlen(dataOut));
    		}
    		else if (*state == 1 && d1_df > x_threshold && abs(d1_dfy) < static_threshold && (msTick - *msTickStateChange) > tau)
    		{
    			*msTickStateChange = msTick;
    			*state = 2;
    			//sprintf(dataOut, "\t\t STATAY 2 \n" );
    			//CDC_Fill_Buffer((uint8_t*)dataOut, strlen(dataOut));
    		}
    		else if (*state == 2 && (d1_df < (-static_threshold) /*|| abs(d1_dfy) > static_threshold*/ || (msTick - *msTickStateChange) > tauMAX) )
    		{
    		 	*msTickStateChange = msTick;
    		    *state = 0;
    		    //sprintf(dataOut, "\t\t GO BACK!!! \n" );
    		...
    		else if (*state == 2 && d1_dfy > (-y_threshold) && abs(d1_df) < static_threshold && (msTick - *msTickStateChange) > tau)
    		{
    			*msTickStateChange = msTick;
    			*state = 3;
    			//sprintf(dataOut, "\t\t State 3 YEEET \n" );
    			//CDC_Fill_Buffer((uint8_t*)dataOut, strlen(dataOut));
    		}
    		else if (*state == 3 && d1_df < (-x_threshold) && abs(d1_dfy) < static_threshold && (msTick - *msTickStateChange) > tau)
    		{
    			*msTickStateChange = msTick;
    			*state = 4;
    			BSP_LED_On(LED1);
    			//sprintf(dataOut, "\t\t EVENT DETECTED!!! \n" );
    			//CDC_Fill_Buffer((uint8_t*)dataOut, strlen(dataOut));
    		}
    		else if (*state == 4 && (msTick - *msTickStateChange) > tauMAX)
    		{
    			*msTickStateChange = msTick;
    			 *state = 0;
    			 BSP_LED_Off(LED1);
